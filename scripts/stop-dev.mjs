...
function parsePort(value = process.env.CODEX_DEV_PORT ?? "3000") {
...
export async function stopDevServer(port = parsePort()) {
  ...
  if (pids.length === 0) {
    console.log(`No AI Ecosystem Atlas server is listening on port ${port}.`);
    return;
  }
  ...
  if (!(await waitForPortToClose(port, 3_000))) {
    signal(listeningPids(port), "SIGKILL");
    if (!(await waitForPortToClose(port, 1_000))) {
      throw new Error(`Could not stop AI Ecosystem Atlas on port ${port}.`);
    }
  }

  console.log(`Stopped AI Ecosystem Atlas on port ${port}.`);
}
